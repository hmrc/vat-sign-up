...
class SubmissionService @Inject()(subscriptionRequestRepository: SubscriptionRequestRepository,
  ...
  def submitSignUpRequest(signUpRequest: SignUpRequest,
    ...
    val email = signUpRequest.signUpEmail collect { case x if x.isVerified => x.emailAddress }
    val isSignUpVerified = signUpRequest.signUpEmail collect { case x if x.isVerified => true }
