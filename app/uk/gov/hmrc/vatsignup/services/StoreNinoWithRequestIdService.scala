...
class StoreNinoWithRequestIdService @Inject()(unconfirmedSubscriptionRequestRepository: UnconfirmedSubscriptionRequestRepository,
  ...
  def storeNino(requestId: String, userDetailsModel: UserDetailsModel, enrolments: Enrolments, ninoSource: NinoSource)
    ...
    ninoSource match {
      case IRSA =>
        storeNinoToMongo(requestId, userDetailsModel.nino, IRSA)
      case UserEntered =>
        matchUser(userDetailsModel, optAgentReferenceNumber) flatMap {
          case Right(nino) => storeNinoToMongo(requestId, nino, UserEntered)
  ...
  private def storeNinoToMongo(requestId: String, nino: String, ninoSource: NinoSource): Future[Either[MongoFailure, StoreNinoSuccess.type]] =
    unconfirmedSubscriptionRequestRepository.upsertNino(requestId, nino, ninoSource) map {
      _ => Right(StoreNinoSuccess)
    } recover {
      case e: NoSuchElementException => Left(NinoDatabaseFailureNoRequestId)
      case _ => Left(NinoDatabaseFailure)
    }

}

object StoreNinoWithRequestIdService {
  ...
  case object NinoDatabaseFailureNoRequestId extends MongoFailure
