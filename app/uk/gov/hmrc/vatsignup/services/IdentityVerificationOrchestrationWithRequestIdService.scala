...
@Singleton
class IdentityVerificationOrchestrationWithRequestIdService @Inject()(unconfirmedSubscriptionRequestRepository: UnconfirmedSubscriptionRequestRepository,
                                                                      identityVerificationConnector: IdentityVerificationConnector
                                                                     )(implicit ec: ExecutionContext) {

  import uk.gov.hmrc.vatsignup.services.IdentityVerificationOrchestrationWithRequestIdService._

  def updateIdentityVerificationState(requestId: String,
                                      journeyLink: String,
                                      confidenceLevel: ConfidenceLevel
                                     )(implicit hc: HeaderCarrier): Future[IdentityVerificationOrchestrationWithRequestIdResponse] = {
    for {
      _ <- checkIdentityVerification(journeyLink, confidenceLevel)
      _ <- upsertIdentityVerified(requestId)
  ...
  private def upsertIdentityVerified(requestId: String): EitherT[Future, IdentityVerificationOrchestrationFailure, IdentityVerified.type] =
    EitherT(unconfirmedSubscriptionRequestRepository.upsertIdentityVerified(requestId)
      .map(_ => Right(IdentityVerified))
      .recover { case _ => Left(IdentityVerificationDatabaseFailure) })
}

object IdentityVerificationOrchestrationWithRequestIdService {

  type IdentityVerificationOrchestrationWithRequestIdResponse = Either[IdentityVerificationOrchestrationFailure, IdentityVerified.type]
