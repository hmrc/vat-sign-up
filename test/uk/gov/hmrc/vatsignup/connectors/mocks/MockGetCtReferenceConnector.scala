...
trait MockGetCtReferenceConnector extends BeforeAndAfterEach with MockitoSugar {
  ...
  override def beforeEach(): Unit = {
    super.beforeEach()
    reset(mockCtReferenceConnector)
  }

  val mockCtReferenceConnector: GetCtReferenceConnector = mock[GetCtReferenceConnector]

  def mockGetCtReference(crn: String)(response: Future[GetCtReferenceResponse]): Unit =
    when(mockCtReferenceConnector.getCtReference(
